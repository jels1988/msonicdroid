...
public class IndiceEjecucionAnioActivity extends ActivityGroupBase {
	...
	@Inject ConsultarIndiceEjecucionAnioProxy consultarIndiceEjecucionAnioProxy;
	...
	protected void process() {
		...
		consultarIndiceEjecucionAnioProxy.setPeriodo(this.periodo);
		consultarIndiceEjecucionAnioProxy.execute();
	}
	...
	public void execute(Context context) {
		if(!consultarIndiceEjecucionAnioProxy.isExito()){
			if(consultarIndiceEjecucionAnioProxy.getResponse().getStatus()!=0){
		...
		if(consultarIndiceEjecucionAnioProxy.getResponse().getData()==null){
			finish();
		}
		
		String[] titles = new String[] { "EJE", "INV","POS","PRE" };
		
		List<IndiceEjecucionAnioTO> data = consultarIndiceEjecucionAnioProxy.getResponse().getData();
		...
		String[] anios = new String[size];
		
		for (IndiceEjecucionAnioTO indiceEjecucionAnioTO : data) {
			eje[index]=ArrayUtil.round(indiceEjecucionAnioTO.getEjecucion(), 0);
			inv[index]=ArrayUtil.round(indiceEjecucionAnioTO.getInventario(), 0);
			pos[index]=ArrayUtil.round(indiceEjecucionAnioTO.getPosicion(), 0);
			pre[index]=ArrayUtil.round(indiceEjecucionAnioTO.getPresentacion(), 0);
			anios[index]=String.valueOf(indiceEjecucionAnioTO.getAnio());
			index++;
		}
	    ...
	    setChartSettings(renderer, titulo_grafico, "A–os", "", 0,3, 0,max * 1.05, 
	    ...
	    for (String anio : anios) {
	    	renderer.addTextLabel(indexanio++, anio);
		}
