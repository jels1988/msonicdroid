...
public class ResumenGerenteVentasActivity extends net.msonic.lib.sherlock.ActivityBase implements ActionBar.TabListener  {
  ...
  @Override
	protected void processOk() {
		...
		boolean isExito = resumenGerenteVentasProxy.isExito();
		if (isExito) {
			int status = resumenGerenteVentasProxy.getResponse().getStatus();
			if (status == 0) {
				
				ArrayList<ResumenVentaTO> resumenVenta = resumenGerenteVentasProxy.getResponse().detalle.resumen;
				ResumenVenta_Adapter adapterResumen = new ResumenVenta_Adapter(this, resumenVenta);
				lstResumen.setAdapter(adapterResumen);
				
				ArrayList<ResumenVentaDetalleTO> resumenVentaDetalle = resumenGerenteVentasProxy.getResponse().detalle.detalle;
				...
				message = resumenGerenteVentasProxy.getResponse().getDescripcion();
				super.processOk();
				showToast(message);
			}
	...
	protected void processError() {
		String message;
		if(resumenGerenteVentasProxy.getResponse()!=null){
			String error = resumenGerenteVentasProxy.getResponse().getDescripcion();
			message = error;
		}else{
	...
	public boolean onCreateOptionsMenu(com.actionbarsherlock.view.Menu menu) {
		...
		getSupportMenuInflater().inflate(R.menu.resumen_gerente_ventas_menu, menu);
